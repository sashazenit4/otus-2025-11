...
class BookGrid extends \CBitrixComponent implements Controllerable, Errorable
{
    ...
    private function prepareGridData(): void
    {
        ...
        // Выгрузка только по тем колонкам, которые выбраны в данный момент
    ...
    private function prepareFilter(array $filterData): array
    {
        ...
        if (!empty($filterData['AUTHOR'])) {
            $filter['AUTHORS.ID'] = $filterData['AUTHOR'];
        }
    ...
    {
        $authorItems = $this->getAuthorFilter();
        return [
            [
                'id' => 'TITLE',
                'name' => Loc::getMessage('BOOK_GRID_BOOK_TITLE_LABEL'),
    ...
    /**
     * @todo Реализовать получение списка авторов
     */
    private function getAuthorFilter(): array
    {
        $authors = AuthorTable::getList(['select' => ['ID', 'FIRST_NAME', 'LAST_NAME']]);
        $toReturn = [];
        foreach ($authors as $author) {
            $toReturn[$author['ID']] = sprintf('%s %s', $author['FIRST_NAME'], $author['LAST_NAME']);
        }
