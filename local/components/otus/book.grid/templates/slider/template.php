...
Extension::load(['otus.modal.dialog', 'date', 'ajax', 'ui.buttons', 'aclips.ui-grid-collapse']);
?>
<div class="filter-container__wrapper">
    ...
    <div class="button__wrapper" id="book_grid_buttons"></div>
</div>
<?php
...
<script>
    function redirectToExcel() {
        ...
        window.open('/excel-export.php?EXPORT_MODE=Y', '_self');
    }
</script>
...
<script>
    ...
    ?>
        var btn = new BX.UI.Button({
            text: '<?= $button['text'] ?>',
            color: '<?= $button['color'] ?>',
            click: '<?= $button['click'] ?>',
        });

        var buttonNode = BX('book_grid_buttons');
        btn.renderTo(buttonNode);
    <?php
    }
    ?>
</script>
